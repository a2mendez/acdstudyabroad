...
$(function () {
  var $container = $('[data-simple-accordion="container"]'),
      $trigger = $('[data-simple-accordion="trigger"]'),
      $content = $('[data-simple-accordion="content"]');

  $trigger.on('click', function (e) {
    e.preventDefault();
    $(this).toggleClass('active animate');
    $(this).closest($container).find($content).not(':animated').first().slideToggle();
  });
});
...
$(function () {
  $('.js-filter').delegate('input[type=radio]', 'change', function () {
    var $lis = $('.filter__item'),
...
$(function () {
  var filterInput = $('.js-filter input[type=radio]'),
      filterButton = $('.js-filter-button');

  filterInput.on('change', function () {
    var filterValue = $('input:radio[name=program-filter]:checked').attr('rel'),
        hrefValue = '/academics/areas-of-study?program-filter=' + filterValue;

    filterButton.attr('href', hrefValue);
  });
});
...
$(function () {
  ...
  var programFilter = getParameterByName('program-filter');

  $('.js-filter').find('input[rel=' + programFilter + ']').click();
});

// Character Count

$(function () {
  $('#input_quote[maxlength]').each(function () {
    var $this = $(this);
    var maxLength = parseInt($this.attr('maxlength'));
    $this.attr('maxlength', null);

    var el = $('<span id="acd-character-count">' + maxLength + '</span>');
    el.insertAfter($this);
    ...
    $this.bind('keyup', function () {
      ...
      var count = maxLength - cc;

      if (count < 0) {
        el.removeClass().addClass('acd-character-count--error');
      } else if (count <= 15) {
        el.removeClass().addClass('acd-character-count--warn');
      } else {
        el.removeClass().addClass('acd-character-count--good');
      }
    });
  });
});
...
$('.acd-carousel-container').slick();

// FILE INPUT NAME

$(function () {
  $('.acd-input-file').each(function () {
    var $input = $(this),
    ...
    $input.on('change', function (e) {
      if (window.FileReader && window.File && window.FileList && window.Blob) {
        ...
        if (file.size > 2 * 1024 * 1024) {
          alert("This image is too large. Only images that are 2MB or smaller can be uploaded.");
          return false;
        }
      }
...
$(function () {
  $('.acd-tabs__content').hide();

  $('[data-simple-tab="nav"]').each(function () {
    var $this = $(this),
        $tabItem = $this.find('[data-simple-tab="nav-item"].active'),
        $link = $tabItem.find('a'),
        $tabContent = $($link.attr('href'));

    $this.on('click', '[data-simple-tab="nav-item"] a', function (e) {
      e.preventDefault();
      var $link = $(this);
      var id = this.hash;
      ...
      if (id && !$link.is('.active')) {
        ...
        $tabContent = $(id).show();
        $tabItem = $link.parent().addClass('active');
      }
    });
  });
});
...
$(function () {
  var $select = $('[data-state-source]');

  $select.click(function () {
    var $value = $(this).data('state-source');

    $(this).closest('body').find('[data-state-target="' + $value + '"]').prop('disabled', false);
  });
});
...
$(function () {
  var $checkbox = $('.acd-group-select [type="checkbox"]'),
...
$(function () {
  var $ddTrigger = $('[data-dropdown-trigger="dd-trigger"]'),
      $ddContainer = $('[data-dropdown-container="dd-container"]'),
      $ddMenu = $('[data-dropdown-effect="dd-submenu"]');

  $($ddMenu).hide();

  $ddTrigger.on('click', function () {
    $(this).closest($ddContainer).siblings($ddContainer).find($ddMenu).addClass('is-hidden');
    $(this).closest($ddContainer).siblings($ddContainer).find($ddTrigger).removeClass('active');
    $(this).toggleClass('active');
    $(this).closest($ddContainer).find($ddMenu).slideToggle();
    return false;
  });
...
$(function () {
  ...
  $('[data-src]').each(function () {
    ...
    $button.click(function () {
      ...
      $el.find('[data-vex-close]').each(function () {
